...
while out_iters < 1:
    ...
    while i < max_iters:
        ...
        X1,X2,X3,X4 = lift_y[:37],lift_y[1:38],lift_y[2:39],lift_y[3:40]
        y1, y2, y3, y4 = y[:37], y[1:38], y[2:39], y[3:40]
        loss = torch.sum((X2-model(X1))**2)+torch.sum((X3-model(model(X1)))**2)\
               +torch.sum((dec_y-y)**2) \
                +torch.sum((Dec(model(X1))-y2)**2)+torch.sum((Dec(model(model(X1)))-y3)**2)
    ...
    np.save('./data/dlko_{}_{}'.format(n,sigma),pred_y)
    Y = pred_y
    plt.subplot(121)
