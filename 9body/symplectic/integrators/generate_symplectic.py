...
from symplectic_adjoint import odeint_symplectic_adjoint as odeint
...
class manybody(nn.Module):
    ...
    def forward(self, t, x):
        ...
        print('current time:',t.item(),end-start,len(time))
        if len(time)>2:
            print('time for per step',time[-1]-time[-2])
        return dx
...
def fix_config(num=6,min_radius=0.9,max_radius=1.2):
    ...
    for i in range(1,num):
        p = output[i*2-2:i*2]
        output[i*2:i*2+2] = rotate2d(p,theta=2*np.pi/(num*2))
...
t = torch.linspace(0, 15, 100)
func = manybody(num)
true_y = odeint(func, y0, t)[:,0,:].detach().numpy() # default: rtol=1e-7, atol=1e-9
# true_y = odeint_adjoint(func, y0, t,method='euler',options=dict(step_size=1e-4))[:,0,:].detach().numpy() # default: rtol=1e-7, atol=1e-9
# true_y = odeint(func, y0, t,rtol=1e-7, atol=1e-10)[:,0,:].detach().numpy() # default: rtol=1e-7, atol=1e-9

end = timeit.default_timer()
print(true_y.shape,end-start)
# np.save('true_{}_3_1000_symplectic_r7_a9'.format(num),true_y) # atol=1e-10: a10
plt.plot(t,true_y[:,0])
plt.show()
