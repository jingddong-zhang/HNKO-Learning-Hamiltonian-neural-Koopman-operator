...
data = np.load('./data/samples_144_501.npy',allow_pickle=True).item()
t = data['t']
x = data['x']
true_y = data['u_x']
...
while out_iters < 1:
    ...
    # g1 = lift_Net(64,120,64+lift_d,k,42)
    # Dec = Decoder(64+lift_d,120,64)
    q = int((dim+20)/2)
    model = K_Net(dim+20,dim+20)
    # model = K2_Net(24, 24)
    H1 = int(1.2*dim)
    g1 = lift_Net(dim, H1, dim+20,k,q)
    ...
    while i < max_iters:
        ...
        lift_y = g1(train_y)
        dec_y = Dec(lift_y)
        X1,X2,X3,X4,X5 = lift_y[:N],lift_y[1:N+1],lift_y[2:N+2],lift_y[3:N+3],lift_y[4:N+4]
        ...
        loss = torch.sum((X2-model(X1))**2)+torch.sum((X3-model(model(X1)))**2)+torch.sum((X4-model(model(model(X1))))**2) \
               + torch.sum((X4 - model(model(model(model(X1))))) ** 2)\
