...
while out_iters < 11:
    ...
    while i < max_iters:
        ...
        # X1,X2 = re_data(y,lift_y)
        # v = g1.v[:,0] / torch.sqrt(torch.sum(g1.v[:,0]**2))
        # v1 = g1.v[:,0]/torch.sqrt(torch.sum(g1.v[:,0]**2))
        # v2 = g1.v[:,1]/torch.sqrt(torch.sum(g1.v[:,1]**2))
        ...
        loss = torch.sum((X2-model(X1))**2)\
               ...
               +torch.sum(torch.mm(lift_y,v)**2)\
                +torch.sum(V**2)
    # +torch.sum((torch.sum(v1*embed_y,dim=1))**2)\
        # loss = torch.sum((X2-model(X1))**2)
        # v = g1.v/torch.sqrt(torch.sum(g1.v**2))
        # loss = torch.sum((torch.sum(y**2,dim=1)+torch.sum(g1(y)**2,dim=1)-g1.k)**2)\
...
np.save('./data/hnko_noise_data',noise_data)
plt.show()