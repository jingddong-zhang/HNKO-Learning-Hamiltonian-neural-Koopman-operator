...
def plot():
    ...
    plt.subplots_adjust(left=0.07, bottom=0.15, right=0.98, top=0.9, hspace=0.95, wspace=0.4)
    sigma_list = [0.0,0.0001,0.0004,0.0009]

    sigma = 0.0
    K = np.load('./data/feature_data/koopman_{}_{}.npy'.format(D_in,sigma))
    # model = K_Net(len(K), len(K))
    # K = model.recurrent_kernel.weight.detach().numpy()
    enc = np.load('./data/feature_data/enc_{}_{}.npy'.format(D_in,sigma))
    ...
    bar_width = 0.5
    plt.bar(np.arange(len(index))+bar_width*0,var_list,color=color_list, width=bar_width)#, color=seven_color[0], alpha=0.6, lw=2.0, edgecolor=seven_color[0],label='No Noise')
    plt.ylim(-8,0.5)
    ...
    # for i in range(len(K)):
    #     # plt.subplot(1,7,i+1)
    #     weight = featurevector[:,i]
    #     # weight = np.random.normal(0,1,[len(K)])
    #     # weight *= 1/np.linalg.norm(weight,ord=2)
    #     quantity = (np.sum(weight * enc, axis=1))
    #     # quantity = enc[:,i]
    #     plt.plot(np.arange(len(quantity)),quantity,label='{}'.format(i))
    #     plt.ylim(-2,2)
    #     plt.legend()
